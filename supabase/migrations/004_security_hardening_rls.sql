...
CREATE POLICY "Admins can update their tenant"
  ...
  USING (id = public.get_tenant_id() AND public.get_role() = 'admin');
...
CREATE POLICY "Admins can manage tenant profiles"
  ...
  USING (tenant_id = public.get_tenant_id() AND public.get_role() = 'admin');
...
CREATE POLICY "Managers/Admins can manage products"
  ON products FOR ALL
  USING (tenant_id = public.get_tenant_id() AND public.get_role() IN ('admin', 'manager'));

-- 6. INVENTORY
CREATE POLICY "Tenant inventory access"
  ON inventory FOR SELECT
  USING (product_id IN (SELECT id FROM products WHERE tenant_id = public.get_tenant_id()));

CREATE POLICY "Managers/Admins can update inventory"
  ON inventory FOR ALL
  USING (product_id IN (SELECT id FROM products WHERE tenant_id = public.get_tenant_id()) 
         AND public.get_role() IN ('admin', 'manager'));

-- 7. SALES & SALE ITEMS
CREATE POLICY "Tenant sales access"
  ON sales FOR ALL
  USING (branch_id IN (SELECT id FROM branches WHERE tenant_id = public.get_tenant_id()));

CREATE POLICY "Tenant sale items access"
  ON sale_items FOR ALL
  USING (sale_id IN (SELECT id FROM sales WHERE branch_id IN (SELECT id FROM branches WHERE tenant_id = public.get_tenant_id())));
