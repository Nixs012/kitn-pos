...
export default function FinancePage() {
  ...
  const fetchFinanceData = async () => {
    ...
    try {
      ...
      // Note: Supabase RLS policies usually handle tenant isolation, but we'll be explicit if needed.
      // Since 'sales' table has 'branch_id' which links to 'branches' which has 'tenant_id'.
      ...
      sales?.forEach(sale => {
        ...
        sale.sale_items?.forEach((item: any) => {
          ...
          p.cogs += cogs;
          p.profit += profit;
          p.margin = (p.profit / p.revenue) * 100;
        });
      });
